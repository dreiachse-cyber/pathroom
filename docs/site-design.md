...
| MVP公開 | 120個前後 | 検索・カテゴリ・詳細・コピー・保存を実用レベルで確認する |
...
ライセンスは公開前の必須決定事項とする。商用利用、改変、再配布、クレジット、アイコン単体の再販売可否を曖昧にしない。
...
- 検査済みSVGを100個以上掲載し、目標120個へ到達している
...
残る公開前の主要決定は、アイコン素材のライセンスと初期アイコン様式の最終確認である。

現在の実装に掲載した120件は、検索・絞り込み・コピー・増分表示を検証するためのTabler Icons由来プレビューであり、PATHROOM独自アイコンの確定版ではない。
